var Quiz = function(name, id, description, questions, time, section, type) {
  this.name = name;
  this.id = id;
...
Quiz.prototype.save = function(_this) {
  ...
  savedData.id = "";
  savedData.timeStamp = getDateTime();
  ...
  if (_this.type === 'random' || this.type === "custom") {
    savedData.questions = _this.questions;
  }
  ...
  alert("Quiz Saved. \n\nNote: you must refresh the page to see any saved sessions" );
}
...
Quiz.prototype.appendSavedSessions = function() {
  ...
  function addSessionBtn(session) {
    ...
    console.log("caching " + quizSessions);
    var sessionBtn = document.createElement("div");
    var resumeBtn = createEl("button", "resumeBtn", session.timeStamp );
...
Quiz.prototype.setDefault = function(session_data) {
  ...
  if (session_data === undefined) {
    ...
    if (this.type === 'random' || this.type === "custom") {
      this.questions = session_data.questions;
    }
  };
};
...
Quiz.prototype.appendNextButton = function() {
  ...
  button.onclick = function() {
    if (_this.type === "custom") {
      // add default questions to the custom quiz
    }
...
Quiz.prototype.append = function(section) {
  var _this = this;
  var allTags = questions.getAllTags();
  ...
  html +=   "<tr><th>Questions</th><td id='q-total'>" + this.questions.length + "</td></tr>";
  html +=   "<tr><th>Status</th><td class='quiz-status'>" + this.status + "</td></tr>";
  html += "</table>";
  if (this.type === "custom") {
    html += "<select class='tag-select' id='tags-" + this.id + "'>";
    for (var i = 0; i < allTags.length; i++) {
		    html += "<option value='" + allTags[i] + "'>" + allTags[i].toUpperCase() + "</option>";
    }
    html += "</select>";
  }
  ...
  // Bind methods to options select
  if (this.type === "custom") {
    var el = document.getElementById("tags-" + _this.id);
    var qEl = document.querySelector("#" + _this.id + " #q-total");

    el.onchange = function() {
      var qs = questions.getByTagName(this.value, "icnd2");
      _this.questions = qs;
      qEl .innerHTML = _this.questions.length;
      _this.maxScore = _this.questions.length;
    }
  }
  // Display page
  this.pages.show(0);
